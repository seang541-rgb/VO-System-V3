...
When a new user signs up, they need a `user_credits` row with `credits_balance = 20` (20 free audits). This should be handled by a database trigger on `auth.users` insert, or manually via SQL.
...
5. Create trigger to give new users 20 free credits
