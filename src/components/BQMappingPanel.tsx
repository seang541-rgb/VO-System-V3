    ...
    <div className="flex flex-col border-t border-slate-700 bg-slate-900">
      <div className="shrink-0 border-b border-slate-800 px-4 py-4">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div>
            <div className="text-xs font-black uppercase tracking-[0.3em] text-emerald-400">{t('bqPanel.title')}</div>
            <div className="mt-2 max-w-3xl text-sm text-slate-400">{t('bqPanel.description')}</div>
          </div>
          <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
            <div className="rounded-xl border border-emerald-900/70 bg-emerald-950/20 px-4 py-3"><div className="text-[11px] uppercase tracking-[0.2em] text-emerald-300">{t('bqPanel.bqMounted')}</div><div className="mt-2 text-2xl font-black text-white">{mappedLabelCount}/{mappingCandidatesCount}</div></div>
            <div className="rounded-xl border border-red-900/70 bg-red-950/20 px-4 py-3"><div className="text-[11px] uppercase tracking-[0.2em] text-red-300">{t('bqPanel.unmapped')}</div><div className="mt-2 text-2xl font-black text-white">{orphanRows.length}</div></div>
            <div className="rounded-xl border border-amber-900/70 bg-amber-950/20 px-4 py-3"><div className="text-[11px] uppercase tracking-[0.2em] text-amber-300">{t('bqPanel.pendingRates')}</div><div className="mt-2 text-2xl font-black text-white">{totalPendingRates}</div></div>
            <div className="rounded-xl border border-blue-900/70 bg-blue-950/20 px-4 py-3"><div className="text-[11px] uppercase tracking-[0.2em] text-blue-300">{t('bqPanel.contractRated')}</div><div className="mt-2 text-2xl font-black text-white">{contractBqCount}</div></div>
          </div>
        </div>
        <div className="mt-4 grid gap-3 xl:grid-cols-[minmax(0,1.35fr)_minmax(18rem,0.65fr)]">
          <div className="rounded-xl border border-slate-700 bg-slate-800/70 px-4 py-3 text-sm text-slate-300">{t('bqPanel.awardedBq')}: <span className="font-semibold text-white">{bqFileName ? `${bqFileName} | ${bqItems.length} ${t('sidebar.lineItemsReady')}` : t('bqPanel.notLoaded')}</span>{bqError && <div className="mt-2 text-red-400">{bqError}</div>}{mappingError && <div className="mt-2 text-red-300">{mappingError}</div>}</div>
          <div className="rounded-xl border border-slate-700 bg-slate-800/70 px-4 py-3 text-sm text-blue-300">{t('bqPanel.statusLabel')}: {compareMessage}</div>
        </div>
        {orphanRows.length > 0 && (
          <div className="mt-4 rounded-xl border border-red-900 bg-red-950/40 px-4 py-4">
            <div className="text-sm font-black uppercase tracking-[0.2em] text-red-200">{t('bqPanel.unmappedItems')}: {orphanInstanceCount} - {t('bqPanel.potentialStarRate')}</div>
            <div className="mt-2 text-sm text-red-300">{orphanRows.length} {t('bqPanel.unmappedWarning')} {orphanPreview ? `${t('bqPanel.examples')}: ${orphanPreview}` : ""}</div>
          </div>
        )}
      </div>
      <div className="px-4 pb-6">
        <div className="flex flex-col rounded-xl border border-slate-700 bg-slate-800/80 shadow-2xl shadow-black/30">
          <div className="shrink-0 border-b border-slate-700 bg-slate-900/80 px-5 py-4">
            <div className="text-sm font-black uppercase tracking-[0.28em] text-white">{t('bqPanel.consoleTitle')}</div>
            <div className="mt-2 text-sm text-slate-400">{t('bqPanel.consoleDesc')}</div>
          </div>
            ...
            <table className="min-w-[110rem] border-collapse text-left text-sm">
              <thead className="sticky top-0 z-20 bg-slate-900 text-xs uppercase tracking-[0.18em] text-slate-400">
                <tr className="border-b border-slate-700">
                  <th className="sticky left-0 z-30 min-w-[24rem] border-r border-slate-700 bg-slate-900 px-3 py-2 font-black text-white">{t('bqPanel.qsDescription')}</th>
                  <th className="min-w-[6rem] px-3 py-2 font-black">{t('bqPanel.section')}</th>
                  <th className="min-w-[7rem] px-3 py-2 font-black">{t('bqPanel.instances')}</th>
                  <th className="min-w-[9rem] px-3 py-2 font-black">{t('bqPanel.systemUnit')}</th>
                  <th className="min-w-[9rem] px-3 py-2 font-black">{t('bqPanel.bqUnit')}</th>
                  <th className="min-w-[16rem] px-3 py-2 font-black">{t('bqPanel.suggestedBq')}</th>
                  <th className="min-w-[18rem] px-3 py-2 font-black">{t('bqPanel.bqItemRef')}</th>
                  <th className="min-w-[14rem] px-3 py-2 font-black">{t('bqPanel.bqDescription')}</th>
                  <th className="min-w-[10rem] px-3 py-2 font-black">{t('bqPanel.contractRate')}</th>
                  <th className="min-w-[12rem] px-3 py-2 font-black">{t('sidebar.status')}</th>
                  <th className="min-w-[12rem] px-3 py-2 font-black">{t('bqPanel.confirmMount')}</th>
                </tr>
              </thead>
              <tbody>
                {mappingRows.map((row) => {
                  ...
                  return (
                    <tr key={row.label} className={`border-b border-slate-700/40 align-top text-sm even:bg-slate-800/20 ${stagedMismatch ? "bg-red-950/25" : row.mappedReference ? "bg-slate-800/35" : "bg-slate-900/10"}`}>
                      <td className={`sticky left-0 z-10 min-w-[24rem] border-r border-slate-700/50 px-3 py-2 ${stagedMismatch ? "bg-red-950 text-red-100" : "bg-slate-900 text-slate-100"}`}>
                        ...
                        <div className="mt-1 text-xs text-slate-500">{t('bqPanel.liveInstances')}: {row.instanceCount}</div>
                      </td>
                      ...
                      <td className="border-r border-slate-700/40 px-3 py-2">
                        {row.suggestedItem ? (
                          <div className="space-y-2">
                            ...
                            <button type="button" className={`rounded-lg border px-3 py-2 text-sm font-semibold transition ${row.suggestedSelectableItem ? "border-slate-600 bg-transparent text-slate-200 hover:border-emerald-500/60 hover:text-emerald-200" : "border-red-800 bg-transparent text-red-300"}`} onClick={() => row.suggestedSelectableItem ? onUpdateMapping(row.label, row.suggestedSelectableItem.itemReference) : undefined}>{row.suggestedSelectableItem ? t('bqPanel.approveSuggestion') : t('bqPanel.suggestionBlocked')}</button>
                          </div>
                        ) : <span className="text-slate-500">{t('bqPanel.noRecommendation')}</span>}
                      </td>
                      <td className="border-r border-slate-700/40 px-3 py-2"><select className={`w-full rounded-lg border px-3 py-2 text-sm font-semibold text-white outline-none transition ${stagedMismatch ? "border-red-500 bg-red-950/50 focus:border-red-400" : "border-slate-700 bg-slate-900 focus:border-blue-500"}`} value={row.draftReference} onChange={(event) => onStageDraft(row.label, event.target.value)}><option value="">{t('bqPanel.unmapped')}</option>{row.selectableItems.map((item) => <option key={item.itemReference} value={item.itemReference}>{item.itemReference}{item.mismatch ? ` [${t('bqPanel.unitMismatch')}: ${item.unit} vs ${row.systemUnit || "-"}]` : ""}</option>)}</select></td>
                      ...
                      <td className="px-3 py-2"><div className="flex flex-col gap-2"><button type="button" className={`rounded-lg px-3 py-2 text-sm font-black transition ${stagedMismatch ? "cursor-not-allowed bg-slate-700 text-slate-300" : "bg-blue-600 text-white hover:bg-blue-500"}`} onClick={() => onUpdateMapping(row.label, row.draftReference)} disabled={!row.draftReference || stagedMismatch}>{stagedMismatch ? t('bqPanel.blocked') : t('bqPanel.confirmMount')}</button>{stagedMismatch && <div className="text-xs font-semibold text-red-300">{t('bqPanel.unitMismatch')}</div>}<button type="button" className="rounded-lg border border-slate-700 bg-transparent px-3 py-2 text-xs font-semibold text-slate-300 hover:bg-slate-800" onClick={() => onUpdateMapping(row.label, "")}>{t('bqPanel.clear')}</button></div></td>
                    </tr>
                  );
                })}
                {mappingRows.length === 0 && <tr><td colSpan={11} className="px-4 py-10 text-center text-sm text-slate-500">{t('bqPanel.runCompareFirst')}</td></tr>}
              </tbody>
            </table>
          </div>
        </div>
