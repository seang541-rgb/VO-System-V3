        ...
        <div className="space-y-1.5">
          <button type="button" onClick={onRunAudit} disabled={!canAudit}
            ...
            <div className="flex-1 overflow-hidden">
              ...
              <div className="truncate text-[10px] text-amber-200">{t('sidebar.runAuditSub')}</div>
            </div>
          </button>
        ...
        <div className="space-y-1 text-[11px]">
          <div className="flex items-center justify-between">
            ...
            <span className="flex items-center gap-1">
              {v1State === 'loading' ? <><Loader2 className="h-3 w-3 animate-spin text-blue-400" /><span className="text-blue-400">{t('sidebar.stateParsing')}</span></> : v1State === 'error' ? <><AlertCircle className="h-3 w-3 text-red-400" /><span className="text-red-400">{t('sidebar.stateError')}</span></> : v1Components.length > 0 ? <><CheckCircle2 className="h-3 w-3 text-emerald-400" /><span className="text-emerald-400">{t('sidebar.stateReady')}</span></> : <><Circle className="h-3 w-3 text-slate-600" /><span className="text-slate-500">{t('sidebar.statePending')}</span></>}
            </span>
          </div>
          <div className="flex items-center justify-between">
            <span className="text-slate-400">{t('sidebar.revisionIfc')}</span>
            <span className="flex items-center gap-1">
              {v2State === 'loading' ? <><Loader2 className="h-3 w-3 animate-spin text-blue-400" /><span className="text-blue-400">{t('sidebar.stateParsing')}</span></> : v2State === 'error' ? <><AlertCircle className="h-3 w-3 text-red-400" /><span className="text-red-400">{t('sidebar.stateError')}</span></> : v2Components.length > 0 ? <><CheckCircle2 className="h-3 w-3 text-emerald-400" /><span className="text-emerald-400">{t('sidebar.stateReady')}</span></> : <><Circle className="h-3 w-3 text-slate-600" /><span className="text-slate-500">{t('sidebar.statePending')}</span></>}
            </span>
          </div>
          <div className="flex items-center justify-between">
