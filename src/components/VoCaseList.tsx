        ...
        <div className="mt-0.5 flex items-center gap-3 text-[10px] text-slate-500">
          ...
          {snapshot && (
            <>
              ...
              <span>{snapshot.total_changes} 个变更</span>
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h3 className="text-sm font-semibold text-white">VO Cases</h3>
          <p className="mt-0.5 text-[10px] text-slate-500">
            {cases.length} 个 Case{activeCases.length !== cases.length && ` (${activeCases.length} 活跃)`}
          </p>
        </div>
        <button
          type="button"
      ...
      {showCreateInput && (
        <div className="flex gap-2 rounded-xl border border-blue-800/50 bg-blue-950/20 p-3">
          ...
          >
            {isCreating ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : '创建'}
          </button>
          <button
            type="button"
        ...
        <>
          {activeCases.length === 0 && cancelledCases.length === 0 ? (
            <div className="py-12 text-center">
              ...
              <p className="mt-3 text-xs text-slate-500">还没有 VO Case</p>
              <p className="mt-1 text-[10px] text-slate-600">
                点击"新建 Case"开始你的第一次 VO 分析
              </p>
            </div>
          ) : (
              ...
              {cancelledCases.length > 0 && (
                <details className="group">
                  <summary className="cursor-pointer text-[10px] text-slate-600 hover:text-slate-400">
                    {cancelledCases.length} 个已取消的 Case
                  </summary>
